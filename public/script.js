document.addEventListener('DOMContentLoaded', () => {
    ...
    mapContainer.addEventListener('mousemove', (e) => {
        ...
        if (e.clientX < mapRect.left || e.clientX > mapRect.right ||
            ...
            // Pozycja myszki względem obrazu
            const mouseX = e.clientX - mapRect.left;
            const mouseY = e.clientY - mapRect.top;
            
            // Wymiary obrazu w naturalnej skali
            const naturalWidth = 10000;
            const naturalHeight = 5500;

            // Obliczanie koordynatów świata gry, gdzie (0,0) jest na środku
            const gameX = Math.round(((mouseX / mapImage.width) * naturalWidth) - (naturalWidth / 2));
            const gameZ = Math.round(((mouseY / mapImage.height) * naturalHeight) - (naturalHeight / 2));
