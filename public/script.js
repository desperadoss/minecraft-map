document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchAllowedSessions() {
        try {
            ...
            if (response.ok) {
                ...
                showError(sessions.message || 'Failed to fetch allowed sessions.');
            }
        } catch (err) {
    ...
    function renderAllowedSessions(sessions) {
        ...
        if (sessions.length === 0) {
            allowedSessionsList.innerHTML = '<li class="no-sessions">No sessions found.</li>';
            return;
        }
        ...
        sessions.forEach(session => {
            ...
            li.innerHTML = `
                ...
                <span class="session-date">Added: ${date}</span>
                <button class="button delete-session-btn" data-code="${session.sessionCode}">Remove</button>
            `;
    ...
    async function deleteSession(code) {
        try {
            ...
            if (response.ok) {
                ...
                fetchAllowedSessions(); // Refresh list after deletion
            } else {
                showError(data.message || 'Failed to remove session.');
            }
        } catch (err) {
    ...
    // Event listener for dynamic delete buttons
